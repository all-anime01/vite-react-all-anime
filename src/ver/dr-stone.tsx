  ...
  5: [
    {
      ...
      videoUrl: 'https://all-anime.net/frame/Dr-stone/T4/1.html',
    },
    {
      id: 2,
      title: '2. Science Journey',
      duration: '23 min',
      description:
      ...
      episodeNumber: 'Sub | Dob',
      videoUrl: 'https://all-anime.net/frame/Dr-stone/T4/2.html',
    },
    {
      id: 3,
      title: '3. Trampa de luz en la oscuridad',
      duration: '23 min',
      description:
      ...
      thumbnail:
        'https://imgsrv.crunchyroll.com/cdn-cgi/image/fit=contain,format=auto,quality=70,width=320,height=180/catalog/crunchyroll/022f5906068b0ab2f3da856e6672bf9c.jpg',
      releaseDate: 'Enero 23, 2025',
      episodeNumber: 'Sub | Dob',
      videoUrl: 'https://all-anime.net/frame/Dr-stone/T4/3.html',
    },
  ],
  4: [
    {
  ...
  3: [
    {
      ...
      thumbnail:
        'https://imgsrv.crunchyroll.com/cdn-cgi/image/fit=contain,format=auto,quality=70,width=320,height=180/catalog/crunchyroll/3db5cb5828101210b5f1e39360e2eb8b.jpg',
...
function DrstoneTV() {
  ...
  const lastSeason = Math.max(...Object.keys(allEpisodes).map(Number));

  // Establecer la última temporada como la seleccionada por defecto
  ...
  useEffect(() => {
    document.title = 'Ver Dr. Stone Online | all-anime';
  }, []);
  ...
  // Función para obtener episodios adyacentes
  const getAdjacentEpisodes = (episodeId: number, season: number) => {
    const episodes = allEpisodes[season as keyof typeof allEpisodes];
    const index = episodes.findIndex((ep) => ep.id === episodeId);

    const previousEpisode = index > 0 ? episodes[index - 1] : null;
    const nextEpisode = index < episodes.length - 1 ? episodes[index + 1] : null;
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                >
                  <option value={5}>T4: Science Future</option>
                  <option value={4}>T3: New World</option>
                  <option value={3}>Episodio especial - RYUSUi</option>
  ...
  <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
    <div className="bg-[#181818] rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto relative">
      <button
      ...
      <div className="p-4 md:p-8">
        ...
        {/* Episodios relacionados - Estilo Crunchyroll */}
        <div className="mt-8">
          <h3 className="text-xl font-semibold mb-4">Episodios relacionados</h3>
          <div className="space-y-3">
            {/* Episodio anterior */}
            {getAdjacentEpisodes(selectedEpisode.id, selectedSeason).previousEpisode && (
              <div
                onClick={() => handleEpisodeClick(getAdjacentEpisodes(selectedEpisode.id, selectedSeason).previousEpisode!)}
                className="flex items-center gap-4 bg-[#2b2b2b] rounded-lg overflow-hidden hover:bg-[#363636] transition-all cursor-pointer p-3"
              >
                <div className="relative w-24 h-14 flex-shrink-0">
                  <img
                    src={getAdjacentEpisodes(selectedEpisode.id, selectedSeason).previousEpisode!.thumbnail}
                    alt={getAdjacentEpisodes(selectedEpisode.id, selectedSeason).previousEpisode!.title}
                ...
                <div className="flex-1">
                  <h4 className="text-sm font-semibold line-clamp-1">
                    {getAdjacentEpisodes(selectedEpisode.id, selectedSeason).previousEpisode!.title}
                  </h4>
                  <p className="text-xs text-gray-400 mt-1">
                    {getAdjacentEpisodes(selectedEpisode.id, selectedSeason).previousEpisode!.episodeNumber}
                  </p>
                </div>
              </div>
            )}
            ...
            {/* Episodio siguiente */}
            {getAdjacentEpisodes(selectedEpisode.id, selectedSeason).nextEpisode && (
              <div
                onClick={() => handleEpisodeClick(getAdjacentEpisodes(selectedEpisode.id, selectedSeason).nextEpisode!)}
                className="flex items-center gap-4 bg-[#2b2b2b] rounded-lg overflow-hidden hover:bg-[#363636] transition-all cursor-pointer p-3"
              >
                <div className="relative w-24 h-14 flex-shrink-0">
                  <img
                    src={getAdjacentEpisodes(selectedEpisode.id, selectedSeason).nextEpisode!.thumbnail}
                    alt={getAdjacentEpisodes(selectedEpisode.id, selectedSeason).nextEpisode!.title}
                    className="w-full h-full object-cover rounded-md"
                  />
                  <div className="absolute inset-0 bg-black/50 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                    <Play className="w-6 h-6 text-white" />
                ...
                <div className="flex-1">
                  <h4 className="text-sm font-semibold line-clamp-1">
                    {getAdjacentEpisodes(selectedEpisode.id, selectedSeason).nextEpisode!.title}
                  </h4>
                  <p className="text-xs text-gray-400 mt-1">
                    {getAdjacentEpisodes(selectedEpisode.id, selectedSeason).nextEpisode!.episodeNumber}
                  </p>
                </div>
              </div>
            )}
            ...
            dangerouslySetInnerHTML={{
              __html: `
                (function() {
                  ...
                  s.src = 'https://
                  all-anime-comment.disqus.com/embed.js';
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-[#181818] rounded-lg w-full max-w-4xl relative">
            ...
            <div className="p-4 md:p-8">
              <div className="relative aspect-video mb-6">
                <iframe
                  src="https://www.youtube.com/embed/pkMgTN4Joi4?si=3qeOufd6k7_kxJEd"
                  className="w-full h-full absolute inset-0"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                ></iframe>
...
4.8 (127.1k)
              ...
              <div className="grid md:grid-cols-2 gap-4 md:gap-8 text-xs md:text-sm text-gray-400">
                <div>
                  ...
                  <p>
                    <span className="text-gray-200">Creador:</span> Boichi, Riichiro Inagaki
                  </p>
                </div>
                <div>
                  <p>
                    <span className="text-gray-200">Géneros:</span> Acción, Aventura, Comedia, Drama, Sci-FI, Shounen
                  </p>
                  <p>
                    <span className="text-gray-200">Aviso de contenido:</span>{' '}
                    +14 Desnudez, Lenguaje ofensivo, Violencia
                  </p>
                </div>
              </div>
            </div>
