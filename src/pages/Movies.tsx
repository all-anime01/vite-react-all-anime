...
import { getAnimeByType } from '../data/animeData';
...
export default function Movies() {
  ...
  const [filteredMovies, setFilteredMovies] = useState<AnimeData[]>(getAnimeByType('Película'));
  const [searchQuery, setSearchQuery] = useState('');

  useEffect(() => {
    document.title = 'All Anime - Películas';
  }, []);

  useEffect(() => {
    let results = getAnimeByType('Película');
  ...
  }, [selectedGenres, selectedYear, searchQuery]);
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-24">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">Películas de Anime</h1>
        <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {filteredMovies.map(movie => (
          ...
          >
            <div className="quality-badge">4K</div>
            <img
              src={movie.image}
              alt={movie.title}
            ...
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg">
              <div className="absolute bottom-0 p-4">
                ...
                <p className="mt-2 text-sm line-clamp-2">{movie.synopsis}</p>
              </div>
            </div>
          </div>
        ))}
